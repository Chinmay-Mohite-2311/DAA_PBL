...
out = cv2.VideoWriter(
    "output.avi", cv2.VideoWriter_fourcc("M", "J", "P", "G"), 10, (width, height)
)
...
class Wall:
    ...
    # drawn a wall object. takes passage as an argument, and draws it in light blue/background
        ...
        if passage == 1:
            ...
            if self.orientation == 1:
                ...
                canvas.create_rectangle(
                    ...
                    self.second_cell[1] * CELL_SIZE + LINE_THICKNESS / 2,
                    (self.first_cell[0] + 1) * CELL_SIZE - LINE_THICKNESS / 2 - 1,
                    fill=color,
                    outline=color,
                )
        else:
            if self.orientation == 1:
                ...
                canvas.create_rectangle(
                    ...
                    self.second_cell[1] * CELL_SIZE + LINE_THICKNESS / 2 + 1,
                    (self.first_cell[0] + 1) * CELL_SIZE + LINE_THICKNESS / 2,
                    fill=color,
                    outline=color,
                )
    ...
    def __eq__(self, other):
        if other is not None:
            if (
                ...
                and self.first_cell[0] == other.first_cell[0]
                and self.first_cell[1] == other.first_cell[1]
                and self.second_cell[1] == other.second_cell[1]
                and self.second_cell[0] == other.second_cell[0]
            ):
                return True
...
class Maze:
    def __init__(self, canvas, starting_cell, demonstration_mode, print_pictures):
        self.canvas = canvas
        self.passage_walls = self.generate_maze(
            starting_cell, demonstration_mode, print_pictures
        )
    ...
    def generate_maze(
        self, starting_cell, demonstration_mode=False, print_pictures=False
    ):
        ...
        while len(active_walls) > 0:
            ...
            self.draw_maze(
                active_walls, passage_walls, maze_cells, wait, active_wall=active_wall
            )
            #
            if active_wall.first_cell not in maze_cells:
                ...
                added_cell = Cell(
                    active_wall.second_cell[0], active_wall.second_cell[1]
                )
                for i in added_cell.Walls:
                    active_walls.append(i)
                passage_walls.append(active_wall)
    ...
    def draw_maze(
        self, active_walls, current_passage_walls, cells, wait=0.0, active_wall=None
    ):
    ...
    def explore(self, row, col, path):
        ...
        if (
            ...
            or row >= GRID_HEIGHT
            or col >= GRID_WIDTH
            or position in self.visited_positions
        ):
            return False
...
def print_picture(walls):
    ...
    for i in walls:
        if i.orientation == 1:
            top_left = (
                int(i.second_cell[1] * CELL_SIZE + LINE_THICKNESS // 2 + 1),
                int(i.second_cell[0] * CELL_SIZE - LINE_THICKNESS // 2),
            )
            bottom_right = (
                int((i.second_cell[1] + 1) * CELL_SIZE - LINE_THICKNESS // 2 - 1),
                int(i.second_cell[0] * CELL_SIZE + LINE_THICKNESS // 2),
            )
        else:
            top_left = (
                int(i.second_cell[1] * CELL_SIZE - LINE_THICKNESS // 2),
        ...
        cv2.rectangle(
            image, top_left, bottom_right, color=(255, 255, 255), thickness=-1
        )

    # maze bounds around the whole maze
    cv2.rectangle(
        image, (0, 0), (width, height), color=(0, 0, 0), thickness=LINE_THICKNESS
    )
    return image
